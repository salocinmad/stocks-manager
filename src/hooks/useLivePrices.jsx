...
export function useLivePrices({ finnhubApiKey, getActivePositions, fetchPriceFromYahoo, operations }) {
    ...
    const pricePromises = companies.map(async (positionKey) => {
      ...
      try {
        ...
        if (priceData && typeof priceData.price === 'number') {
          ...
          // Check for stop loss
          if (stopLossPrice && priceData.price <= stopLossPrice) {
            console.log(`Stop Loss triggered for ${companyName} (${symbol}) at ${priceData.price}. Stop Loss price: ${stopLossPrice}`)
            try {
              await notificationAPI.sendStopLossEmail({ company: companyName, symbol, currentPrice: priceData.price, stopLossPrice });
            } catch (notificationError) {
              console.error('Error sending stop loss email notification:', notificationError);
            }
          }
        }
  ...
  }, [finnhubApiKey, getActivePositions, fetchPriceFromYahoo, operations])
