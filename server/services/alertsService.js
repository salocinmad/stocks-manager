...
export async function checkLossSustained(position, userId, portfolioId, days = 30, threshold = 15) {
    ...
    try {
        const currentPrice = await db.query.priceCaches.findFirst({
            where: and(
                eq(schema.priceCaches.userId, userId),
                eq(schema.priceCaches.portfolioId, portfolioId),
                eq(schema.priceCaches.positionKey, position.positionKey)
            )
        });
        ...
        const priceNDaysAgo = await db.query.dailyPrices.findFirst({
            where: and(
                ...
                eq(schema.dailyPrices.date, dateNDaysAgo.toISOString().split('T')[0])
            )
        });
