...
export async function fillHistoricalGaps(symbol, maxDaysBack = HISTORICAL_CONFIG.DEFAULT_DAYS) {
    ...
    const recordCountResult = await db.select({ count: count() })
        .from(schema.globalStockPrices)
        .where(eq(schema.globalStockPrices.symbol, symbol));
    const recordCount = Number(recordCountResult[0].count);
...
export async function getPriceForDate(symbol, date) {
    return await db.query.globalStockPrices.findFirst({
        where: and(
            eq(schema.globalStockPrices.symbol, symbol),
            eq(schema.globalStockPrices.date, date)
        )
    });
}
