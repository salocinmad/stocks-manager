...
export async function generateDailyReport(userId, portfolioId, date, currentEURUSD = null) {
    ...
    try {
        ...
        for (const positionKey in activePositions) {
            ...
            const existingDailyPrices = await db.select().from(schema.dailyPrices)
                .where(and(
        ...
        const priceCaches = await db.select().from(schema.priceCaches).where(and(eq(schema.priceCaches.userId, userId), eq(schema.priceCaches.portfolioId, portfolioId)));
        ...
        const dailyStats = await db.select().from(schema.dailyPortfolioStats).where(and(
            eq(schema.dailyPortfolioStats.userId, userId),
            eq(schema.dailyPortfolioStats.portfolioId, portfolioId),
            gte(schema.dailyPortfolioStats.date, thirtyDaysAgoStr),
            lte(schema.dailyPortfolioStats.date, date)
        )).orderBy(asc(schema.dailyPortfolioStats.date));
