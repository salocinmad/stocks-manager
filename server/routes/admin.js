...
router.post('/users', async (req, res) => {
  try {
    const { username, password, isAdmin: adminFlag = false } = req.body;
    if (!username || !password) return res.status(400).json({ error: 'Usuario y contraseña son requeridos' });
    const lowerUsername = username.toLowerCase();
    const existingUser = await db.select().from(schema.users).where(eq(schema.users.username, lowerUsername)).limit(1);
    if (existingUser.length > 0) return res.status(400).json({ error: 'El usuario ya existe' });
    const saltRounds = 12;
...
router.post('/finnhub-api-key', async (req, res) => {
  try {
    ...
    const [config] = await db.insert(schema.configs).values({
      key: 'finnhub-api-key',
      value
    }).onConflictDoUpdate({
      target: schema.configs.key,
      set: { value: sql`excluded.value` }
    }).returning({ value: schema.configs.value });
...
router.get('/smtp-pass', async (req, res) => {
  try {
    const result = await db.select().from(schema.configs).where(eq(schema.configs.key, 'smtp_pass'));
    const cfg = result[0];
...
router.get('/operations/:portfolioId', async (req, res) => {
  try {
    ...
    const operations = await db.select().from(schema.operations)
      .where(eq(schema.operations.portfolioId, portfolioId))
      .orderBy(desc(schema.operations.date));
    res.json(operations);
  } catch (error) {
...
router.post('/scheduler', async (req, res) => {
  try {
    ...
    if (enabled !== undefined) {
      ...
      const existing = await db.select().from(schema.configs).where(eq(schema.configs.key, key)).limit(1);
