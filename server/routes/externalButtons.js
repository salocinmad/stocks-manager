...
import { eq, asc, count, and, ne } from 'drizzle-orm';
...
router.get('/', async (req, res) => {
    try {
        const buttons = await db.select().from(schema.externalLinkButtons).where(
            eq(schema.externalLinkButtons.userId, req.user.id)
        ).orderBy(asc(schema.externalLinkButtons.displayOrder));
        res.json(buttons);
    } catch (error) {
